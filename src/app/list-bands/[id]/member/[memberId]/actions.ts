...
import { updateMemberHTTP } from '@/app/http/update-member'
...
export async function updateMemberAction(data: FormData) {
  const response = updateMemberSchema.safeParse(Object.fromEntries(data))
  ...
  const { id, name, email, bandRoleId, office, avatar } = response.data
